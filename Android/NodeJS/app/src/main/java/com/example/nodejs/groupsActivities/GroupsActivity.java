...
import com.example.nodejs.utils.GroupsRecycleViewAdapter;
...
public class GroupsActivity extends AppCompatActivity implements GroupsRecycleViewAdapter.ItemClickListener {

    FinanceBackend myAPI;
    Gson gson = new GsonBuilder().setLenient().create();
    CompositeDisposable compositeDisposable = new CompositeDisposable();
    GroupsRecycleViewAdapter GroupsRecycleViewAdapter;

    public void addCatalogueJsonArrayToRecyclerView(RecyclerView recyclerView, JsonArray catalogueJsonArray) {
        GroupsRecycleViewAdapter = new GroupsRecycleViewAdapter(GroupsActivity.this, catalogueJsonArray);
        GroupsRecycleViewAdapter.setClickListener(GroupsActivity.this);
        GroupsRecycleViewAdapter.setHasStableIds(true);
        recyclerView.setAdapter(GroupsRecycleViewAdapter);
    }

    private void cancelUpdate() {
        Intent myIntent = new Intent(GroupsActivity.this, FinanceGroupsActivity.class);
        GroupsActivity.this.startActivity(myIntent);
        finish();
    }

    private void startItemActivity(){
        Intent myIntent = new Intent(GroupsActivity.this, GroupsItemViewActivity.class);
        this.startActivity(myIntent);
    }

    private boolean isThereStoredCatalogue() {
        final SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(GroupsActivity.this);
        final String userString = settings.getString("Groups", "{}");
    ...
    public void onItemClick(View view, int position) {
        SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(GroupsActivity.this);
        GroupsItem catalogueItem = GroupsRecycleViewAdapter.getItem(position);
        settings.edit().putString("GroupsItem", gson.toJson(catalogueItem)).apply();
        startItemActivity();
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final LinearLayout linearLayout = new LinearLayout(GroupsActivity.this);
        ...
        if (isThereStoredCatalogue()){
            JsonArray catalogueJsonArray = gson.fromJson(settings.getString("Groups", "{}"), JsonArray.class);
        ...
        catalogueListButton.setOnClickListener(v -> {
            ...
            myAPI = retrofit.create(FinanceBackend.class);
            compositeDisposable.add(myAPI.getGroups(bearerToken)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(response -> {
                        if (response.code() >= 200 && response.code() < 300) {
                            JsonArray allUsersJsonArray = response.body().getAsJsonArray("groups");
                            settings.edit().putString("groups", gson.toJson(allUsersJsonArray)).apply();
                            ...
                            Toast.makeText(GroupsActivity.this, "Groups list retrieve successful.", Toast.LENGTH_LONG).show();
                        } else {
                            Toast.makeText(GroupsActivity.this, response.code() + " " + response.errorBody().string(), Toast.LENGTH_LONG).show();
                        }
                    }));
        });
